...
public interface JDBCConnector extends Connectable {
  Logger LOGGER = Loggers.getLogger(
      "de.xab.porter.transfer.jdbc.datasource.JDBCDataSource");

  @Override
  default java.sql.Connection connect(DataConnection dataConnection,
                                      Object dataSource) {
    java.sql.Connection connection;
    HikariDataSource hikariDataSource = (HikariDataSource)dataSource;
    try {
      LOGGER.log(Level.INFO, String.format("connecting to %s %s...",
                                           dataConnection.getType(),
  ...
  default void close(Object connection, Object dataSource) {
    HikariDataSource hikariDataSource = (HikariDataSource)dataSource;
    LOGGER.log(Level.INFO, String.format("closing connection to %s...",
  ...
  default String getJDBCUrl(DataConnection dataConnection) {
    ...
    return String.format("jdbc:%s://%s/%s", getType(), dataConnection.getUrl(),
                         schema);
  }
}
