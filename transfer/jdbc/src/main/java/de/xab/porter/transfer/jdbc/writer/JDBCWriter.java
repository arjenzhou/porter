...
public class JDBCWriter extends AbstractWriter implements JDBCConnector {
  ...
  @Override
  public void createTable(DataConnection dataConnection, Object connection,
                          Result<?> data) {
    ...
    logger.log(Level.FINE, String.format("meta of table %s is: \n%s",
                                         tableIdentifier, Jsons.toJson(meta)));
    // ddl
    ...
    try (Statement stmt = ((Connection)connection).createStatement()) {
      ...
      logger.log(Level.INFO,
                 String.format("table %s create failed", tableIdentifier));
      throw new PorterException("create table failed", e);
    }
  }

  @Override
  protected void doWrite(DataConnection dataConnection, Object connection,
                         Result<?> data) {
   ...
   */
  protected void writeInValueMode(Connection connection, Relation relation,
    ...
    if (!properties.isAllColumns()) {
      sqlBuilder.append("(")
          .append(relation.getMeta()
                      .stream()
                      .map(column
                           -> getColumnIdentifier(column.getName(),
                                                  properties.getQuote()))
                      .collect(Collectors.joining(", ")))
          .append(")");
    }
    sqlBuilder.append("VALUES\n");
    sqlBuilder.append(relation.getData()
                          .stream()
                          .map(row
                               -> "(" +
                                      row.stream()
   ...
   */
  protected void
  writeInStatementBatchMode(Connection connection, Relation relation,
                            SinkConnection.Properties properties) {
    String tableIdentifier = properties.getTableIdentifier();
    StringBuilder sqlBuilder =
   ...
   */
  protected void writeInPrepareBatchMode(Connection connection,
                                         Relation relation,
                                         SinkConnection.Properties properties) {
    String tableIdentifier = properties.getTableIdentifier();
    StringBuilder sqlBuilder =
  ...
  protected void writeInDefaultMode(SinkConnection sinkConnection,
    ...
    writeInValueMode(jdbcConnection, (Relation)data.getResult(),
                     sinkConnection.getProperties());
  }
  ...
  public void dropTable(DataConnection dataConnection, Object connection) {
    ...
    try (Statement stmt = ((Connection)connection).createStatement()) {
      ...
      logger.log(Level.INFO,
                 String.format("drop table %s: %s", tableIdentifier, ddl));
      stmt.executeUpdate(ddl);
    } catch (SQLException e) {
   ...
   * generate create DDL
   */
  protected String getCreateDDL(String tableIdentifier, String quote,
                                List<Column> meta) {
    return getCreate(tableIdentifier) + getColumns(meta, quote) +
  ...
  /**
   * keep primary key order with origin table/view constraint, sorted by
   * primaryKeySeq
   */
  protected Map<Short, String> sortPrimaryKey(List<Column> meta, String quote) {
    ...
    meta.forEach(column -> {
      ...
      if (column.isPrimaryKey() && primaryKeySeq != 0) {
        primaryKeyMap.put(primaryKeySeq,
                          getColumnIdentifier(column.getName(), quote));
      }
    });
