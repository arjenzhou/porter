...
public class JDBCReader extends AbstractReader implements JDBCConnector {
  ...
  @Override
  public void doRead(SrcConnection srcConnection, Object connection,
    ...
    try {
      ...
      while (resultSet.next()) {
        ...
        if (batch % DEFAULT_BATCH_SIZE == 0) {
          logger.log(Level.INFO, String.format("read %d rows from %s %s", batch,
                                               srcConnection.getType(),
                                               srcConnection.getUrl()));
      ...
      if (!relation.getData().isEmpty()) {
        // only one batch
        seq = seq == FIRST.getSequenceNum() ? FIRST_AND_LAST.getSequenceNum()
      ...
      logger.log(Level.INFO, String.format("read last %d scrap(s) from %s %s",
                                           relation.getData().size(),
                                           srcConnection.getType(),
                                           srcConnection.getUrl()));
      this.pushToChannel(new Result<>(seq, relation));
    } catch (SQLException exception) {
      throw new PorterException("read data from JDBC connection failed",
                                exception);
    } finally {
      ...
      logger.log(
          Level.INFO,
          String.format(
              "read completed. %s rows have been read, cost %s second(s)",
              batch, seconds));
      try {
        if (statement != null) {
          statement.close();
  ...
  @Override
  public Map<String, Column> getTableMetaData(Context context,
                                              Object connection) {
    SrcConnection srcConnection = context.getSrcConnection();
    Connection jdbcConnection = (Connection)connection;
    // keep insertion order
    Map<String, Column> columnMap = new LinkedHashMap<>();
    try (ResultSet columns = jdbcConnection.getMetaData().getColumns(
             srcConnection.getCatalog(), srcConnection.getSchema(),
             srcConnection.getTable(), null);
         ResultSet primaryKeys = jdbcConnection.getMetaData().getPrimaryKeys(
             srcConnection.getCatalog(), srcConnection.getSchema(),
             srcConnection.getTable());
         ResultSet indexInfo = jdbcConnection.getMetaData().getIndexInfo(
             srcConnection.getCatalog(), srcConnection.getSchema(),
             srcConnection.getTable(), false, false)) {
      ...
      throw new PorterException(
          "read source meta data from JDBC connection failed", exception);
    }
    return columnMap;
  }
   ...
   */
  protected Statement getStatement(Connection jdbcConnection)
      throws SQLException {
    Statement statement =
        jdbcConnection.createStatement(TYPE_FORWARD_ONLY, CONCUR_READ_ONLY);
  ...
  private void fillResultSetMeta(Map<String, Column> columnMap,
                                 SrcConnection.Properties properties,
                                 ResultSetMetaData metaData, int columnCount)
      throws SQLException {
    for (int i = 1; i <= columnCount; i++) {
      String name = metaData.getColumnName(i);
      Column column =
          columnMap.compute(name,
                            (key, oldValue)
                                -> Objects.requireNonNullElseGet(
                                    oldValue, () -> new Column(name)));
