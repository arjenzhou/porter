...
public class DemoTest {
  ...
  private void createH2Table(String table) {
    ...
    final String ddl = "CREATE TABLE " + table + "(id INTEGER not NULL, "
                       + " first VARCHAR(255), "
                       + " last VARCHAR(255), "
                       + " age INTEGER, "
                       + " PRIMARY KEY ( id ))";
  ...
  public void testNewReader() {
    ...
    final SrcConnection srcConn =
        ((SrcConnection.Builder) new SrcConnection.Builder()
             .type(type)
             .username("")
             .password("")
             .schema(schema)
    ...
    final SinkConnection sinkConnection =
        ((SinkConnection.Builder) new SinkConnection.Builder()
             .type(type)
             .username("")
             .password("")
             .schema(schema)
             .table(table + "_tmp"))
            .properties(SinkConnection.Properties.builder()
                            .drop(true)
                            .create(true)
    ...
    try (Connection connection = DriverManager.getConnection(url, "", "");
      ...
      final ResultSet resultSet =
          statement.executeQuery("SELECT * FROM \"PUBLIC\".\"mock_table_tmp\"");
