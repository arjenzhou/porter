...
/**
 * a extension loader can load any implements of one service, which is describe
 * in resources/extensions. ExtensionLoader load class using current class
 * loader, construct new instance and inject type for consuming <p> extension
 * must have these features: implement at least one service, registered in
 * resources/extensions <p> any service must opens to module {@link
 * de.xab.porter.common} <p> comments after # will be ignored
 */
public class ExtensionLoader {
  private static final String FOLDER = "extensions/";
  private static final Map<Class<?>, Map<String, Class<?>>> EXTENSION_HOLDER =
  ...
  public <T> T loadExtension(String type, Class<T> clazz) {
    ...
    if (!driverClass.getModule().isOpen(driverClass.getPackageName(),
                                        this.getClass().getModule())) {
      throw new PorterException(String.format("cannot access class %s at %s",
                                              driverClass.getName(),
                                              this.getClass().getModule()));
    }
    try {
  ...
  public Class<?> loadClass(String type, Class<?> clazz) {
    ...
    try {
      ...
      throw new PorterException(String.format(
          "no implement(s) of %s found in %s", clazz.getName(), fileName));
    }
    while (urls != null && urls.hasMoreElements()) {
      ...
      try (BufferedReader reader = new BufferedReader(new InputStreamReader(
        ...
        String line;
        final Map<String, Class<?>> specifyClassMap =
            EXTENSION_HOLDER.get(clazz);
        ...
        throw new PorterException(
            String.format("load extension class %s failed", clazz.getName()),
            e);
      }
    }
    throw new PorterException(String.format(
  ...
  private Map.Entry<String, String> parseTypeAndClass(String line,
                                                      Class<?> clazz) {
    String driverType;
    String className;
    String newLine = line;
    ...
    throw new PorterException(String.format(
        "parse extension %s failed. expected `type=foo.bar.Extension`, got %s",
   ...
   */
  private Class<?> loadClassByName(ClassLoader loader, Class<?> service,
                                   String className) {
    try {
      ...
      if (implementedInterface(subClass, service)) {
        ...
        throw new PorterException(
            String.format("no implement classes found of class %s", service));
      }
    } catch (ClassNotFoundException e) {
      throw new PorterException(String.format("class %s not found", className),
                                e);
    }
  }
   ...
   */
  private boolean implementedInterface(Class<?> subClass,
  ...
  private <T> void injectExtension(T instance, String type) {
    for (Method method : instance.getClass().getMethods()) {
      ...
      try {
        ...
        throw new PorterException(
            String.format("instance %s inject %s failed", instance, type));
      }
    }
  }
