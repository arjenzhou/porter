...
public class Task {
  ...
  public void init() {
    ...
    this.reader = ExtensionLoader.getExtensionLoader().loadExtension(
        srcConnection.getType(), Reader.class);
  ...
  /**
   * construct relations among reader, writer and its channel, define the action
   * when channel is ready to write
   */
  public void register() {
    final List<SinkConnection> sinkConnections = context.getSinkConnections();
    this.writers =
        sinkConnections.stream()
            .map(sink -> {
              final Writer writer =
                  ExtensionLoader.getExtensionLoader().loadExtension(
                      sink.getType(), Writer.class);
              final Object dataSource = writer.getDataSource(sink);
              final Object connection =
                  writer.connect(sink, writer.getDataSource(sink));
              final Channel channel =
                  ExtensionLoader.getExtensionLoader().loadExtension(
                      this.context.getProperties().getChannel(), Channel.class);
              channel.setOnReadListener(
                  data -> writer.write(connection, dataSource, sink, data));
  ...
  private void registerProperties() {
    final SrcConnection srcConnection = context.getSrcConnection();
    final SrcConnection.Properties srcConnectionProperties =
        srcConnection.getProperties();
    final List<SinkConnection> sinkConnections = context.getSinkConnections();
